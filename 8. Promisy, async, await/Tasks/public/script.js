...
function createCard({ username, email, address }) {
    //* Username
...
function calculateData(item) {
    const flattedDataArr = [];
    const countryData = item.map((country) => ({
    ...
    const withoutCapitol = countryData.filter((el) => {
        if (!el.capitol) {
            let { name } = el;
            console.log(name);
        }
    ...
    const groupByContinent = (arr, key) => {
    ...
    const biggestNumOfCountries = (arr) => {
        let continentsWithNumbers = {};
        ...
        console.log(`The highest countries number has ${sortedContinents[sortedContinents.length - 1].country}, with amount of ${sortedContinents[sortedContinents.length - 1].countries}`);
    };
    ...
    const sumPopulation = (arr) => {
        let sortedPopulation = [];
        for (const num in arr) {
            console.log(arr[num]);
            const populationCountries = arr[num].map((element) => ({
                sumContinent: element.continent,
                population: element.population,
            }));
            const sum = populationCountries.reduce((prev, curr) => (prev += curr.population), 0);
    ...
    const sumArea = (arr) => {
        ...
        for (const area in arr) {
            ...
            const areas = areaObject.map((element) => ({
                continents: element.continent,
                area: element.area,
            }));
            let sum = areas.reduce((prev, curr) => (prev += curr.area), 0);
            arrArea.push(sum);
        }
    ...
    const currencySorter = (arr) => {
        let currencyCode = [];
        let currencies = arr.map((element) => ({
